...
import { TAM_UNG_DETAIL_ROUTE, TAM_UNG_ROUTE, TAM_UNG_ADD_ROUTE } from '../../../helper/constant';
...
export class ListComponent implements OnInit {
  title = `Tạm ứng`;
  addRoute = `${TAM_UNG_ADD_ROUTE}`;
  editRoute = TAM_UNG_DETAIL_ROUTE;
  ...
  async ngOnInit() {
    const maBA = this.route.snapshot.queryParamMap.get('maBA');
  ...
  redirectTaoMoi(): void {
    this.router.navigateByUrl(`/${TAM_UNG_ROUTE}/${TAM_UNG_ADD_ROUTE}?maBA=${this.benhAnSelected.maBA}`);
  }

  async redirectSua(maPTU: string): Promise<void> {
    const phieuTamUng = await this.service.getById(maPTU);
    this.router.navigateByUrl(`/${TAM_UNG_ROUTE}/${TAM_UNG_DETAIL_ROUTE}/${maPTU}?maBA=${phieuTamUng.benhAn.maBA}`);
  }
}
