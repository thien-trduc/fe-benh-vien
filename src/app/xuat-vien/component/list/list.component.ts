...
import { XUAT_VIEN_DETAIL_ROUTE, XUAT_VIEN_ROUTE, XUAT_VIEN_ADD_ROUTE } from '../../../helper/constant';
...
export class ListComponent implements OnInit {
  title = `Xuất Viện`;
  addRoute = `${XUAT_VIEN_ADD_ROUTE}`;
  editRoute = XUAT_VIEN_DETAIL_ROUTE;
  listRoute = XUAT_VIEN_ROUTE;

  datas: XuatVien[] = [];
  searchResult: XuatVien[] = [];
  pageSize = 5;
  pageIndex = 1;
  total = 1;
  ...
  redirectTaoMoi(): void {
    this.router.navigateByUrl(`/${XUAT_VIEN_ROUTE}/${XUAT_VIEN_ADD_ROUTE}?maBA=${this.benhAnSelected.maBA}`);
  }
}
