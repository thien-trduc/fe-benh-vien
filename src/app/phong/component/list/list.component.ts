...
import { PHONG_ADD_ROUTE, PHONG_DETAIL_ROUTE, PHONG_ROUTE } from 'src/app/helper/constant';
...
export class ListComponent implements OnInit {
  title = `Phòng`;
  addRoute = PHONG_ADD_ROUTE;
  editRoute = PHONG_DETAIL_ROUTE;
  ...
  constructor(private service: PhongService, private message: NzMessageService) { }
    ...
    if (value.length > 0) {
      try {
        const [resText, res] = await Promise.all([
          this.service.search(1, 10000, { soPhong: value }),
          this.service.search(this.pageIndex, this.pageSize, { soPhong: value }),
        ]);
    ...
    if (value) {
      try {
        ...
        const {
          ...
          total,
          rows
        } = await this.service.search(this.pageIndex, this.pageSize, { soPhong: value });
        this.datas = rows;
        this.pageIndex = pageIndex;
        this.pageSize = pageSize;
        this.total = total;
